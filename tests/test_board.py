...
def test_should_return_true_when_position_is_occupied_on_board():
    ...
    board.chart[0][0] = PositionStatus.OCCUPIED.value
...
def test_should_return_true_when_position_is_bombed_on_board():
    ...
    board.chart[0][0] = PositionStatus.BOMBED.value
...
def test_should_occupy_free_positions_from_board():
    board = Board2D(4, 4)  # A new board completely free

    occupy_board_positions(board, [Position(1, 1)])

    assert board.chart[1][1] == PositionStatus.OCCUPIED.value


def test_should_raise_exception_that_cannot_occupy_board_position():
    board = Board2D(4, 4)  # A new board completely free

    board.chart[1][1] = PositionStatus.BOMBED.value  # Occupy position (1,1)
